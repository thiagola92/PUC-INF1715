...
expression_and:	expression_and TOKEN_AND expression_equal	{ $$ = create_node(EXPRESSION_AND, 2, $1, $3); }
			          | expression_equal { $$ = $1; }
			          ;

expression_equal:	expression_equal TOKEN_EQUAL expression_not_equal	{ $$ = create_node(EXPRESSION_EQUAL, 2, $1, $3); }
			            | expression_not_equal { $$ = $1; }
			            ;

expression_not_equal:	expression_not_equal TOKEN_NOT_EQUAL expression_greater { $$ = create_node(EXPRESSION_NOT_EQUAL, 2, $1, $3); }
			                | expression_greater { $$ = $1; }
			                ;

expression_greater:	expression_greater TOKEN_GREATER expression_greater_equal	{ $$ = create_node(EXPRESSION_GREATER, 2, $1, $3); }
			              | expression_greater_equal { $$ = $1; }
			              ;

expression_greater_equal:	expression_greater_equal TOKEN_GREATER_EQUAL expression_less { $$ = create_node(EXPRESSION_GREATER_EQUAL, 2, $1, $3); }
				                  | expression_less { $$ = $1; }
				                  ;

expression_less: 	expression_less TOKEN_LESS expression_less_equal { $$ = create_node(EXPRESSION_LESS, 2, $1, $3); }
			            | expression_less_equal { $$ = $1; }
			            ;

expression_less_equal: 	expression_less_equal TOKEN_LESS_EQUAL expression_sub { $$ = create_node(EXPRESSION_LESS_EQUAL, 2, $1, $3); }
			                  | expression_sub { $$ = $1; }
			                  ;

expression_sub:		expression_sub TOKEN_SUBTRACT expression_add { $$ = create_node(EXPRESSION_SUB, 2, $1, $3); }
			            | expression_add { $$ = $1; }
			            ;

expression_add:		expression_add TOKEN_ADD expression_div	{ $$ = create_node(EXPRESSION_ADD, 2, $1, $3); }
			            | expression_div { $$ = $1; }
			            ;

expression_div:		expression_div TOKEN_DIVIDE expression_mult	{ $$ = create_node(EXPRESSION_DIV, 2, $1, $3); }
			            | expression_mult { $$ = $1; }
			            ;

expression_mult:	expression_mult TOKEN_MULTIPLY expression_cast { $$ = create_node(EXPRESSION_MULT, 2, $1, $3); }
			            | expression_cast { $$ = $1; }
			            ;
